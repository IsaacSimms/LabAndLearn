# Notes from Chapter Four - Ambassador Pattern
...
### Value
clean seperation of concerns (important in enterprise. networking team can cleanly handle networking logic and SWEs can cleanly handle app logic. as an example)
...
## Summary
Ambassadors are a container that runs alongside the workload in the same pod and brokers all communication with the external network. It handles complex networking logic such as discovery, routing, and request splitting on behalf of the workload. The standard pattern involves the workload communicating with teh ambassador through localhost. As far as what type of logic it handles, ambassador containers remind me of gateways in Azure in a lot of ways. Although gateways are multi-node, centralized services where ambassador containers are single-node per instance architecture.